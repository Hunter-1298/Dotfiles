...
-- Keymaps
-- Visual mode: yank path + line range + selected code
vim.keymap.set("v", "<leader>yr", function()
	M.yank_visual_with_path(M.get_buffer_cwd_relative(), "relative")
end, { desc = "[Y]ank [R]elative path + line range + code" })

vim.keymap.set("v", "<leader>ya", function()
	M.yank_visual_with_path(M.get_buffer_absolute(), "absolute")
end, { desc = "[Y]ank [A]bsolute path + line range + code" })

-- Normal mode: yank just the file path
vim.keymap.set("n", "<leader>yp", function()
	M.yank_path(M.get_buffer_cwd_relative(), "relative")
end, { desc = "[Y]ank relative file [P]ath" })

vim.keymap.set("n", "<leader>yP", function()
	M.yank_path(M.get_buffer_absolute(), "absolute")
